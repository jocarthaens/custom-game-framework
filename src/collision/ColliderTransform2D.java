...
public class ColliderTransform2D {
	...
	public ColliderTransform2D setPosition(float x, float y) {
		this.posX = x;
		this.posY = y;
		return this;
	}
	...
	public ColliderTransform2D setRotation(double radians) {
		this.rotateX = (float) Math.cos(radians);
		this.rotateY = (float) Math.sin(radians);
		return this;
	}
	
	public ColliderTransform2D setRotation(float cos, float sin) {
	...
	public ColliderTransform2D translate(float x, float y) {
		this.posX += x;
		this.posY += y;
		return this;
	}
	...
	public ColliderTransform2D rotate(double radians) {
		return this.rotate( (float) Math.cos(radians), (float) Math.sin(radians) );
	}
	
	public ColliderTransform2D rotateAroundPoint(double radians, float pointX, float pointY) {
		this.rotate(radians);
	...
	public ColliderTransform2D rotateAroundPoint(float cos, float sin, float pointX, float pointY) {
		// if value is greater than epsilon or less than 0,
		// divide cos and sin by its length before applying rotation
		if ( Math.abs( (cos * cos + sin * sin) - 1) > FLOAT_EPSILON ) { 
			float len = (float) Math.sqrt(cos * cos + sin * sin);
	...
	public ColliderTransform2D rotate(float cos, float sin) {
		// if value is greater than epsilon or less than 0,
		// divide cos and sin by its length before applying rotation
